...
def main():
    ...
    p.add_argument("--project", "-P", type=int, default=1, help="Project number 1..99 (default: 1)")
    p.add_argument("--group",   "-g", default="A", choices=list("ABCDabcd"),
                   help="Group A/B/C/D (default: A)")

    pad_group = p.add_mutually_exclusive_group(required=True)
    pad_group.add_argument("--pad", help=f"Physical pad label, one of: {sorted(PAD_LABEL_TO_NUM)}")
    ...
    time_mode = args.time_mode or ("bpm" if args.bpm is not None else None)

    print(f"\n  Plan:")
    print(f"    {args.wav.name}  →  slot {args.slot}")
    print(f"    Project {args.project}, Group {group}, pad '{pad_label}' (pad_num {pad_num})")
    if args.bpm is not None:
        print(f"    sound.bpm = {args.bpm}")
    if time_mode:
        print(f"    time.mode = {time_mode}")
    if args.playmode:
        print(f"    sound.playmode = {args.playmode}")
    print()
    ...
    with EP133Client.open(inter_message_delay_s=args.delay_ms / 1000.0) as client:
        ...
        client.upload_sample(args.wav, slot=args.slot, name=args.name)
        print(f"done ({time.monotonic()-t0:.1f}s)", flush=True)

        # 2. Slot metadata (only if we have something to write)
        slot_kwargs = {}
        if args.bpm is not None:
            slot_kwargs["bpm"] = args.bpm
        if time_mode is not None:
            slot_kwargs["time_mode"] = time_mode
        if args.playmode is not None:
            slot_kwargs["playmode"] = args.playmode
        ...
        if args.playmode is not None:
            pad_kwargs["playmode"] = args.playmode
        pad_params = PadParams(**pad_kwargs) if pad_kwargs else None

        print(f"  assign  P{args.project} {group}-{pad_label} (pad_num={pad_num}) → slot {args.slot} ...",
              end=" ", flush=True)
        client.assign_pad(project=args.project, group=group,
